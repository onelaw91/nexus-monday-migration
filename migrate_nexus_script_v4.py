...
def change_column_value(board_id, item_id, column_id, value):
    """
    Updates one specific column on an existing monday.com item.
...
def get_linked_items_for_deliverable(deliverable_item_id):
    """
    Checks whether a deliverable is linked to an engagement.
    ...
    variables = {
        "item_ids": [str(deliverable_item_id)],
    ...
    return items[0].get("linked_items", [])


def link_engagement_with_verification(deliverable_item_id, engagement_item_id, deliverable_id, source_engagement_id):
    """
    Links a deliverable to its engagement and verifies the link.

    Flow:
    1. Run change_column_value to update the connect-board column
    2. Query monday.com to confirm the linked engagement exists
    3. If the link is missing, retry the update
    ...
    for attempt in range(1, max_link_attempts + 1):
        change_column_value(
            ...
            item_id=deliverable_item_id,
            column_id=DELIVERABLE_COLUMNS["engagement_link"],
            value={
                "item_ids": [engagement_item_id]
            },
        )

        time.sleep(2)

        linked_items = get_linked_items_for_deliverable(deliverable_item_id)
        linked_item_ids = [str(linked_item["id"]) for linked_item in linked_items]

        if str(engagement_item_id) in linked_item_ids:
            return True

        print(
            f"Engagement link not confirmed for {deliverable_id} "
    ...
    raise Exception(
        f"Unable to confirm engagement link for deliverable {deliverable_id} "
        f"to engagement {source_engagement_id} after {max_link_attempts} attempts."
    )
...
def migrate_deliverables(rows, engagement_item_ids):
    ...
    for row in rows:
        ...
        column_values = {
            ...
            # Relationship note:
            # The engagement link is intentionally NOT set inside create_item.
            # Connect-board columns are updated separately below using
            # change_column_value so the relationship is more reliable.
        }
        ...
        # Update and verify the connect-board relationship after the deliverable exists.
        #
        # Demo explanation:
        # "This is the second API step for deliverables. The first step creates
        # the deliverable row. This second step updates the Nexus Engagements
        # connect-board column, then queries monday.com to confirm the link
        # actually saved. If the link is missing, the script retries the update."
        link_engagement_with_verification(
            deliverable_item_id=item["id"],
            engagement_item_id=linked_engagement_item_id,
